...
if (isset($_POST['submit'])) {
  ...
  if (!empty($_POST['email']) && !empty($_POST['pseudo']) && !empty($_POST['password']) && !empty($_POST['password-confirm'])) {
    ...
    $verifmail = "SELECT * FROM users WHERE email = '$email'";
    $result = $connexion->query($verifmail);

    // 3) Insérer le nouvel utilisateur en BDD
    if ($result->num_rows > 0) {
      $error = "Cet email est déjà utilisé.";
    } else {
      $nouvelutilisateur = "INSERT INTO users (email, pseudo, hash_password) VALUES ('$email', '$pseudo', '$hash')";

      $requete = "INSERT INTO `users` VALUES (`$email`,`$pseudo`,`$hash`)";
